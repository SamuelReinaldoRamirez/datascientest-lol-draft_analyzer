#!/usr/bin/env python3
"""
Entrainement des modeles V3 — Summoner Stats Temporelles.

Evolution par rapport a V2:
  1. RE-AJOUT des summoner stats (93 features) calculees TEMPORELLEMENT
     (seuls les matchs anterieurs sont utilises, pas de fuite)
  2. Chargement des parquets temporels (compute_temporal_summoner_stats.py)
  3. Features totales: draft ~152, in-game ~170-185

Pre-requis:
  - Avoir lance: python scripts/compute_temporal_summoner_stats.py
  - Fichiers: data/processed/train_temporal_stats.parquet
...
# ============================================================================
# CHAMPION NAME NORMALIZATION (identical to V2)
# ============================================================================
...
# ============================================================================
# 1. LOAD EXTERNAL DATA (identical to V2)
# ============================================================================
...
def load_simple_winrates():
    ...
    refined_path = os.path.join(
        ...
        'donnees raffinees', 'df_Simple_WR_FULL.csv'
    )
    # Try with accent
...
# ============================================================================
# 2. ADD FEATURES (identical to V2)
# ============================================================================
...
def load_or_compute_synergies(train_df, force_recompute=False):
    ...
    syn_path = os.path.join(PROJECT_ROOT, 'data', 'features', 'champion_synergy_counter_v3.pkl')

    if os.path.exists(syn_path) and not force_recompute:
        print("  Chargement synergies/counters existantes (v3)...")
        with open(syn_path, 'rb') as f:
            return pickle.load(f)

    print("  Calcul des synergies/counters depuis les matchs d'entrainement...")
...
# ============================================================================
# 3. DERIVED TIMELINE FEATURES (identical to V2)
# ============================================================================
...
# ============================================================================
# 4. FEATURE SELECTION (V3 — includes temporal summoner stats)
# ============================================================================

def get_summoner_features(df):
    """
    Get summoner stat features (temporal, no leakage).
    93 features total:
      - 80 per-position (8 features x 5 positions x 2 teams)
      - 10 team aggregates (5 x 2 teams)
      - 3-4 differentials
    """
    features = []

    summoner_patterns = [
        '_role_pct', '_role_winrate', '_mastery_points',
        '_streak_type', '_streak_length',
        '_role_kda', '_role_vision', '_champ_recent_wr',
        '_avg_role_specialization', '_min_role_specialization',
        '_avg_role_winrate', '_total_mastery_points', '_streak_momentum',
    ]

    summoner_exact = [
        'role_specialization_diff', 'role_winrate_diff',
        'streak_momentum_diff', 'mastery_diff',
    ]

    for col in df.columns:
        for p in summoner_patterns:
            if p in col and col not in features:
                features.append(col)
                break

    for col in summoner_exact:
...
def get_draft_features(df):
    """
    Get draft features — V3: external winrates + matchups + synergies + summoner stats.
    """
    features = []

    # External winrate features (from dpm.lol)
    ext_patterns = [
    ...
    # V3: Summoner stats features (temporal)
    summoner_feats = get_summoner_features(df)
    for col in summoner_feats:
...
# ============================================================================
# 5. MODEL TRAINING (identical to V2)
# ============================================================================
...
def train_model(X_train, X_test, y_train, y_test, model_name, strong_regularization=False):
    ...
    strong_regularization=True for draft model.
    """
        ...
        if val_acc > best_val:
            ...
            best = {
                ...
                'metadata': {
                    ...
                    'version': 'v3_temporal_summoner_stats',
                }
            }
...
def main():
    parser = argparse.ArgumentParser(description='Train V3 models (temporal summoner stats)')
    parser.add_argument('--no-synergy-recompute', action='store_true')
    args = parser.parse_args()

    print("=" * 70)
    print("ENTRAINEMENT V3 — SUMMONER STATS TEMPORELLES")
    print("=" * 70)
    print("\nEvolutions par rapport a V2:")
    print("  [NEW] Re-ajout des summoner stats (93 features) calculees temporellement")
    print("  [NEW] Chargement des parquets enrichis (compute_temporal_summoner_stats.py)")
    print("  [KEPT] Winrates externes dpm.lol + matchups + synergies/counters")
    print("  [KEPT] Split temporel + regularisation renforcee")
    print("=" * 70)

    total_start = time.time()

    # ==================================================================
    # STEP 1: Load temporal parquets
    # ==================================================================
    print("\n[1/7] Chargement des donnees temporelles...")
    train_path = os.path.join(PROJECT_ROOT, 'data', 'processed', 'train_temporal_stats.parquet')
    test_path = os.path.join(PROJECT_ROOT, 'data', 'processed', 'test_temporal_stats.parquet')
    ...
    if not os.path.exists(train_path):
        ...
        print(f"  Lancez d'abord: python scripts/compute_temporal_summoner_stats.py")
        return

    train_df = pd.read_parquet(train_path)
    test_df = pd.read_parquet(test_path)
    print(f"  Train: {len(train_df):,} matchs ({len(train_df.columns)} colonnes)")
    print(f"  Test:  {len(test_df):,} matchs ({len(test_df.columns)} colonnes)")

    # Check summoner stats are present
    summoner_check_cols = ['team_100_top_role_winrate', 'role_winrate_diff', 'team_100_streak_momentum']
    for col in summoner_check_cols:
        if col in train_df.columns:
            print(f"  OK: {col} present (train mean={train_df[col].mean():.4f})")
        else:
            print(f"  WARN: {col} absent!")

    # Merge timeline data
    if os.path.exists(timeline_path):
        print("\n  Merge timeline multi-minutes...")
    ...
    # ==================================================================
    # STEP 4: Feature lists (V3)
    # ==================================================================
    print("\n[6/7] Definition des features V3...")

    target = 'team_100_win'

    # Summoner features diagnostic
    summoner_feats = get_summoner_features(train_df)
    print(f"\n  Summoner features ({len(summoner_feats)}):")
    for f in summoner_feats[:10]:
        print(f"    - {f}")
    if len(summoner_feats) > 10:
        print(f"    ... et {len(summoner_feats) - 10} autres")

    draft_features = get_draft_features(train_df)
    print(f"\n  Draft features total ({len(draft_features)}):")
    ...
    # STEP 5: Train models
    # ==================================================================
    print("\n[7/7] Entrainement des modeles V3...")
    results = {}
    ...
    # ========================
    print(f"\n{'='*60}")
    print("MODELE: DRAFT V3 (summoner stats temporelles + ext winrates)")
    print(f"{'='*60}")
    ...
    if hasattr(result['model'], 'feature_importances_'):
        ...
        for _, row in imp.head(20).iterrows():
            ...
            print(f"    {row['feature']:50} {row['importance']:.4f} ({pct:.1f}%)")

    # Correlation check (V3 diagnostic)
    print(f"\n  --- Diagnostic V3: correlation train vs test ---")
    key_cols = ['role_winrate_diff', 'role_specialization_diff', 'streak_momentum_diff', 'mastery_diff']
    for col in key_cols:
        if col in train_sorted.columns and col in test_sorted.columns:
            tr_corr = train_sorted[col].corr(train_sorted[target])
            te_corr = test_sorted[col].corr(test_sorted[target])
            ratio = abs(tr_corr / te_corr) if te_corr != 0 else float('inf')
            flag = " *** ATTENTION" if ratio > 10 else " OK"
            print(f"    {col:<35} train={tr_corr:.4f}  test={te_corr:.4f}  ratio={ratio:.1f}{flag}")
    ...
    for minute in [5, 10, 15, 20]:
        print(f"\n{'='*60}")
        print(f"MODELE: @{minute}min V3 (split temporel)")
        print(f"{'='*60}")
    ...
    print(f"\n{'='*70}")
    print("RESUME FINAL (V3 — SUMMONER STATS TEMPORELLES)")
    print(f"{'='*70}")
    print(f"\n{'Modele':<10} {'Algo':<12} {'Feat':<6} {'Val Acc':<10} {'Test Acc':<10} {'Test AUC':<10} {'Train N':<10} {'Test N':<10}")
    print("-" * 80)
    for name, result in results.items():
        m = result['metadata']
    ...
    print(f"\nV3 vs V2:")
    print(f"  + 93 summoner features temporelles (role_winrate, KDA, vision, streak, mastery)")
    print(f"  + Calcul temporel par bucket mensuel (pas de fuite)")
    print(f"  = Winrates externes + matchups + synergies/counters inchanges")
    print(f"  = Split temporel + regularisation renforcee inchanges")
