...
class DataCollector:
    ...
    def fetch_and_store_timeline(self, match_id: str, match_detail: dict, collect_events: bool = True) -> bool:
        """
        Fetch timeline for a match and store gold data per minute + events.

        Args:
            match_id: The match ID
            match_detail: The match details (to map participantId to position)
            collect_events: Whether to also collect events (kills, objectives)
                ...
                if collect_events:
                    ...
                    for event in events:
                        ...
                        if event_type == "CHAMPION_KILL":
                            ...
                            # Determine killer team from participantId
    ...
    def export_to_csv(self):
        ...
        try:
            ...
            # Save full dataset
            df.to_csv("data/match_data_from_db.csv", index=False)
            self.logger.info(f"Exported {len(df)} matches to data/match_data_from_db.csv")

            # Save simplified draft-focused dataset
            # Include all columns - no filtering needed since we want everything
            draft_columns = df.columns.tolist()

            df_draft = df[draft_columns]
            df_draft.to_csv("data/draft_data_from_db.csv", index=False)
            self.logger.info(f"Exported draft data to data/draft_data_from_db.csv")
